import docker
from image_utils import calculate_hash, get_stored_hash, register_hash

client = docker.from_env()

def handle_image_create(image_id):
    image_hash = calculate_hash(image_id)
    register_hash(image_id, image_hash)

def handle_container_event(image_id):
    ...
    if current_hash == stored_hash:
        print(f"Integrity check passed for image: {image_id}")
        return True
    else:
        print(f"Integrity check failed for image: {image_id}")
        # 여기에 실패 시 수행할 작업 추가 (예: 컨테이너 중지, 알림 발송 등)
        return False

for event in client.events(decode=True):
    if event['Type'] == 'image' and event['Action'] == 'create':
        handle_image_create(event['id'])
    elif event['Type'] == 'container' and event['Action'] in ['create', 'start']:
        image_id = event['Actor']['Attributes']['Image']
        handle_container_event(image_id)
