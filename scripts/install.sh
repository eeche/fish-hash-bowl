eeche@eeche-virtual-machine:~/Desktop/fish-hash-client-1.0.0/scripts$ cat install.sh 
#!/bin/bash
...
read -p "Enter the server URL (default: http://localhost:8080): " SERVER_URL
...
echo "You can check the service status with: sudo systemctl status docker-event-monitor.service"
echo "And view logs with: sudo journalctl -u docker-event-monitor.service -f"
